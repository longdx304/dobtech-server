...
import { isString, MoneyAmount, PriceListPriceCreateInput, PriceListPriceUpdateInput, Product, ProductVariantMoneyAmount } from '@medusajs/medusa';
import { dataSource } from '@medusajs/medusa/dist/loaders/database';
import { Brackets, In, IsNull, Not, ObjectLiteral, Repository, WhereExpressionBuilder } from 'typeorm';
...
export const MoneyAmountRepository: Repository<MoneyAmount> = dataSource
		...
		async findManyForVariantsInRegion(
			...
			if (customer_id) {
				qb.leftJoin('price_list.customer_groups', 'cgroup')
					...
					.andWhere('(cgroup.id is null AND price_list.customer_id is null OR cgc.customer_id = :customer_id OR price_list.customer_id = :customer_id)', {
						customer_id,
					});
			} else {
				qb.leftJoin('price_list.customer_groups', 'cgroup').andWhere(
					'cgroup.id is null AND price_list.customer_id is null'
				);
			}
